...
fn convert_from_c_string(ptr: *const raw::c_char) -> String {
    unsafe {
        if ptr.is_null() {
            "".to_owned()
        } else {
...
impl NotmuchDb {
    pub fn open(path: &str) -> NotmuchResult<NotmuchDb> {
        ...
        if result != _notmuch_status_NOTMUCH_STATUS_SUCCESS {
            Err(NotmuchError::DbFailedToOpen(convert_from_c_string(msg)))
        } else {
            Ok(NotmuchDb { db_ptr })
        }
    }
...
fn get_header(msg: *mut notmuch_message_t, header: &str) -> String {
    unsafe {
        convert_from_c_string(notmuch_message_get_header(
            msg,
